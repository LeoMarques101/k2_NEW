...
local k2_alavanca_bless_in = MoveEvent()
local k2_alavanca_bless_out = MoveEvent()
...
function k2_alavanca_bless.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    local bless = ALAVANCA_BLESS[item.actionid]

    if(player:hasBlessing(bless.number)) then
        player:sendTextMessage(MESSAGE_INFO_DESCR,"Voce ja tem essa bless!")
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
        return true
    else
        if playermoney >= CUSTO_BLESS then
            player:sendTextMessage(MESSAGE_INFO_DESCR,bless.nome.." concedida!!!")
...
function k2_alavanca_bless_in.onStepIn(creature, item, position, fromPosition)
	if item.itemid == 3216 then
		item:transform(3217)
	end
    local key = item.actionid - 10
    
    local str = ALAVANCA_BLESS[key].nome .." por ".. CUSTO_BLESS .." golds."
...
function k2_alavanca_bless_out.onStepOut(creature, item, position, fromPosition)
...
k2_alavanca_bless_out:aid(1311,1312,1313,1314,1315)
k2_alavanca_bless_in:aid(1311,1312,1313,1314,1315)
k2_alavanca_bless_out:register()
k2_alavanca_bless_in:register()